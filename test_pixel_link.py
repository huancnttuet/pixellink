...
def test():
    ...
    with tf.Session(config = sess_config) as sess:
        ...
        for iter, image_name in enumerate(image_names):
            ...
            pixel_pos_scores, link_pos_scores,pixel_cls_scores,pixel_cls_logits,pixel_link_scores,block_1, block_2, block_3, block_4, block_5, fc6,fc7 = sess.run(
                [net.pixel_pos_scores, net.link_pos_scores, net.pixel_cls_scores, net.pixel_cls_logits, net.pixel_link_scores, net.block1, net.block2,net.block3,net.block4,net.block5,net.fc6,net.fc7], 
                feed_dict = {
                    image:image_data
            })
            print('logits')
            print(pixel_cls_logits[:,:,:,1])
            print('scores')
            print(pixel_cls_scores[:,:,:,1])
            print('block1')
            print(block_1)
            d = pixel_cls_logits[:,:,:,1][0]
            
            print(len(link_pos_scores[0]))
            print(len(link_pos_scores[0][0]))
            print(len(link_pos_scores[0][0][0]))

            plt.imshow(block_1[:,:,:,0][0])
            plt.show()

            plt.imshow(block_2[:,:,:,0][0])
            plt.show()
            plt.imshow(block_3[:,:,:,0][0])
            plt.show()
            plt.imshow(block_4[:,:,:,0][0])
            plt.show()
            plt.imshow(block_5[:,:,:,0][0])
            plt.show()
            plt.imshow(fc6[:,:,:,0][0])
            plt.show()
            plt.imshow(fc7[:,:,:,0][0])
            plt.show()
            plt.imshow(pixel_cls_logits[:,:,:,1][0])
            plt.show()
            plt.imshow(pixel_cls_scores[:,:,:,1][0])
            plt.show()
            ...
            #     plt.imshow(block_1[:,:,:,i][0])
            # plt.show()

            # fig2=plt.figure(figsize=(8, 8))
            # for i in range(0, columns*rows):
            #     fig2.add_subplot(rows, columns, i + 1)
            #     plt.imshow(block_2[:,:,:,i][0])
            # plt.show()

            # fig3=plt.figure(figsize=(8, 8))
            # for i in range(0, columns*rows):
            #     fig3.add_subplot(rows, columns, i + 1)
            #     plt.imshow(block_3[:,:,:,i][0])
            # plt.show()

            # fig4=plt.figure(figsize=(8, 8))
            # for i in range(0, columns*rows):
            #     fig4.add_subplot(rows, columns, i + 1)
            #     plt.imshow(block_4[:,:,:,i][0])
            # plt.show()

            # fig5=plt.figure(figsize=(8, 8))
            # for i in range(0, columns*rows):
            #     fig5.add_subplot(rows, columns, i + 1)
            #     plt.imshow(block_5[:,:,:,i][0])
            # plt.show()

            # fig6=plt.figure(figsize=(8, 8))
            # for i in range(0, columns*rows):
            #     fig6.add_subplot(rows, columns, i + 1)
            #     plt.imshow(fc6[:,:,:,i][0])
            # plt.show()

            # fig7=plt.figure(figsize=(8, 8))
            # for i in range(0, columns*rows):
            #     fig7.add_subplot(rows, columns, i + 1)
            #     plt.imshow(fc7[:,:,:,i][0])
            # plt.show()

            # fig8=plt.figure(figsize=(2, 2))
            # for i in range(0, 2*1):
            #     fig8.add_subplot(rows, columns, i + 1)
            #     plt.imshow(pixel_cls_logits[:,:,:,i][0])
            # plt.show()

            # fig9=plt.figure(figsize=(2, 2))
            # for i in range(0, 2*1):
            #     fig9.add_subplot(rows, columns, i + 1)
            #     plt.imshow(pixel_cls_scores[:,:,:,i][0])
            # plt.show()
            
            # fig10=plt.figure(figsize=(4, 4))
            # for i in range(0, 2*4):
            #     fig10.add_subplot(rows, columns, i + 1)
            #     plt.imshow(link_pos_scores[:,:,:,i][0])
            # plt.show()
